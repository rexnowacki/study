...
void print_locale(struct lconv *locale);
void error_and_exit(const char *message, const char *locale);

int main(int argc, char *argv[]) {
    struct lconv *locale;

    if (argc != 2) {
        error_and_exit("Usage: %s locale\n", argv[0]);
    }

    if (setlocale(LC_ALL, argv[1]) == NULL) {
        error_and_exit("No information available for %s\n", argv[1]);
    }
...
void print_locale(struct lconv *locale) {
    ...
    printf("grouping = \"%d\"\n", (int)locale->grouping[0]);
    printf("mon_decimal_point = \"%s\"\n", locale->mon_decimal_point);
    printf("mon_thousands_sep = \"%s\"\n", locale->mon_thousands_sep);
    printf("mon_grouping = \"%d\"\n", (int)locale->mon_grouping[0]);
